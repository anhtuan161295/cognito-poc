...
import com.serverless.cognito.function.CallPostpaidAPI;
			...
			new ThreadLocal<MessageDigest>() {
				...
				protected MessageDigest initialValue() {
					try {
						...
						LOGGER.error("Error in MessageDigest initialValue method of AuthenticationHelper: ", e);
	...
	public String performSRPAuthentication(String username, String password) {
		...
		try {
			...
			if (org.apache.commons.lang3.StringUtils.isNotBlank(secretKey)) {
				secret = CognitoSecretHash.getSecretHash(username, Settings.getClientId(), Settings.getClientSecret());
			}

			BasicAWSCredentials awsCreds = new BasicAWSCredentials(clientId, secret);
...
//			Gson gson = new Gson();
//			LOGGER.info("InitiateAuthResult : " + gson.toJson(initiateAuthResult));
...
//				System.out.println(CognitoJWTParser.getPayload(result.getAuthenticationResult().getIdToken()));
			...
			LOGGER.error("Error in performSRPAuthentication method of AuthenticationHelper: ", e);
		}
		return authresult;
	}
	...
	private RespondToAuthChallengeRequest userSrpAuthRequest(InitiateAuthResult challenge,
		...
		try {
			...
			LOGGER.error("Error in userSrpAuthRequest method of AuthenticationHelper: " + e);
		}
		...
		if (org.apache.commons.lang3.StringUtils.isNotBlank(secretKey)) {
			srpAuthResponses.put("SECRET_HASH", this.calculateSecretHash(this.clientId, this.secretKey, usernameInternal));
		}
	...
	public String calculateSecretHash(String userPoolClientId, String userPoolClientSecret, String userName) {
		...
		try {
			...
			LOGGER.error("Error in calculateSecretHash method of AuthenticationHelper: ", e);
